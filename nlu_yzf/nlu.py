...
s_way_vocab_path = os.path.join(FLAGS.data_dir, "s_way.txt")
...
def nlu_interface(nlu_inputs, sess, model):
    ...
    hyp_s_way = rev_s_way_vocab[np.argmax(logits[4])]
    hyp_intent = rev_intent_vocab[np.argmax(logits[5])]
...
test = json.dumps({"request_data": {"input": ['是', '的']},
                   "dr_result": {"domain": '翼支付'}})
