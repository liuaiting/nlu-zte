...
def prepare_multi_task_data(data_dir, sent_vocab_size):
    ...
    create_label_vocab(s_way_vocab_path, train_path + "_s_way.txt")
    create_label_vocab(intent_vocab_path, train_path + "_intent.txt")
    ...
    s_way_train_ids_path = train_path + "_ids_s_way.txt"
    ...
    data_to_token_ids(train_path + "_s_ope.txt", s_ope_train_ids_path, s_ope_vocab_path, normalize_digits=False,
                      use_padding=False)
    data_to_token_ids(train_path + "_s_way.txt", s_way_train_ids_path, s_way_vocab_path, normalize_digits=False,
                      use_padding=False)
    ...
    s_way_valid_ids_path = valid_path + "_ids_s_way.txt"
    ...
    data_to_token_ids(valid_path + "_s_ope.txt", s_ope_valid_ids_path, s_ope_vocab_path, normalize_digits=False,
                      use_padding=False)
    data_to_token_ids(valid_path + "_s_way.txt", s_way_valid_ids_path, s_way_vocab_path, normalize_digits=False,
                      use_padding=False)
    ...
    s_way_test_ids_path = test_path + "_ids_s_way.txt"
    ...
    data_to_token_ids(test_path + "_s_ope.txt", s_ope_test_ids_path, s_ope_vocab_path, normalize_digits=False,
                      use_padding=False)
    data_to_token_ids(test_path + "_s_way.txt", s_way_test_ids_path, s_way_vocab_path, normalize_digits=False,
                      use_padding=False)
            ...
            with tf.gfile.GFile(slot_path[1], mode="r") as s_loc_file:
                with tf.gfile.GFile(slot_path[2], mode="r") as s_name_file:
                    with tf.gfile.GFile(slot_path[3], mode="r") as s_ope_file:
                        with tf.gfile.GFile(slot_path[4], mode="r") as s_way_file:
                            with tf.gfile.GFile(intent_path, mode="r") as intent_file:
                                ...
                                s_way = s_way_file.readline()
                                intent = intent_file.readline()
                                counter = 0
                                while sent and s_attr and s_loc and s_name and s_ope and s_way and intent and (
                                    ...
                                    s_attr_ids = [int(x) for x in s_attr.split()]
                                    s_loc_ids = [int(x) for x in s_loc.split()]
                                    s_name_ids = [int(x) for x in s_name.split()]
                                    s_ope_ids = [int(x) for x in s_ope.split()]
                                    s_way_ids = [int(x) for x in s_way.split()]
                                    intent_ids = [int(x) for x in intent.split()]
                                    data_set.append(
                                        [sent_ids, s_attr_ids, s_loc_ids, s_name_ids, s_ope_ids, s_way_ids, intent_ids])
