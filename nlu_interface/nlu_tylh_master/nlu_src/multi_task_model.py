...
class MultiTaskModel(object):
    ...
    def __init__(self,
        ...
        self.slot_loss = (self.s_attr_loss + self.s_loc_loss + self.s_name_loss + self.s_ope_loss) / 4
        ...
        variables = [v for v in tf.global_variables() if v.op.name.startswith('model_tylh')]
    ...
    def step(self, session, inputs, s_attrs, s_locs, s_names, s_opes, intents,
    ...
    def get_batch(self, data):
        ...
        inputs, s_attrs, s_locs, s_names, s_opes, intents = [], [], [], [], [], []
        batch_sequence_length_list = list()

        # Get a random batch of inputs, targets and labels from data,
        # pad them if needed.
        for _ in range(self.batch_size):
            _input, _s_attr, _s_loc, _s_name, _s_ope, _intent = random.choice(data)
        ...
        return batch_inputs, batch_s_attrs, batch_s_locs, batch_s_names, batch_s_opes, batch_intents, batch_sequence_length
    ...
    def get_one(self, data, sample_id):
        ...
        # Get a random batch of inputs, targets and labels from data,
        # pad them if needed.
        _input, _s_attr, _s_loc, _s_name, _s_ope, _intent = data[sample_id]
        batch_sequence_length_list.append(len(_input))
