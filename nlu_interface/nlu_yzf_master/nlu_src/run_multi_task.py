...
from nlu_yzf_master.nlu_src import data_utils
from nlu_yzf_master.nlu_src import multi_task_model
    ...
    with tf.Session(config=config) as sess:
        ...
        while model.global_step.eval() < FLAGS.max_training_steps:
            ...
            _, step_loss, logits = model.step(sess, batch_inputs, batch_s_attrs, batch_s_locs,
                                              batch_s_names, batch_s_opes, batch_s_ways, batch_intents,
            ...
            if current_step % FLAGS.steps_per_checkpoint == 0:
                ...
                def write_eval_result(result_list, result_path):
                    with tf.gfile.GFile(result_path, 'w') as f:
                        for i in range(len(result_list[0])):
                            f.write(' '.join([str(result_list[j][i]) for j in range(6)]) + '\n')
                ...
                test_slot_accuracy, teat_intent_accuracy, hyp_list = run_valid_test(test_set, 'Test')
